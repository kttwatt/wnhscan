-- รหัส 301 = สินทรัพย์ไม่มีตัวตนอื่น (ไม่ใช่ ที่ดิน); ลบหมวด 315 ที่ซ้ำ
BEGIN;

INSERT INTO public.item_subgroups (id, group_id, name, sort_order, created_at, code)
VALUES (
  'b3010301-0000-4000-8000-000000000301'::uuid,
  'b5129ccd-0adf-4386-bb5d-7f3235814278'::uuid,
  'สินทรัพย์ไม่มีตัวตนอื่น',
  1,
  '2026-06-20 15:41:17.391463+00'::timestamptz,
  '301'
)
...
UPDATE public.items
SET subgroup_id = 'b3010301-0000-4000-8000-000000000301'::uuid
WHERE subgroup_id = 'b3010315-0000-4000-8000-000000000315'::uuid;

DELETE FROM public.item_subgroups
WHERE id = 'b3010315-0000-4000-8000-000000000315'::uuid;
